...
(() => {
  ...
  const documentCategories = {
    "Shipping & Transport Documents": ["Bill of Lading (B/L) – Master and House B/L", "Sea Waybill", "Booking Confirmation", "Shipping Instructions", "Arrival Notice", "Container Load Plan (CLP)", "Freight Invoice / Charges Sheet"],
    "Customs Documentation": ["Commercial Invoice", "Packing List", "Certificate of Origin", "Customs Entry / Declaration Forms", "Import/Export Licenses", "HS Classification Documentation", "Customs Release/Disposition Notices"],
    "Regulatory and Compliance Documents": ["Dangerous Goods Declaration (DGD)", "Material Safety Data Sheet (MSDS)", "Fumigation Certificate", "Inspection Certificates", "ISF Filing Confirmation", "AMS/ACI/ENS Filing Confirmations"],
    "Trade & Financial Documents": ["Letter of Credit (if applicable)", "Payment Receipts", "Proof of Delivery (POD)", "Purchase Order", "Sales Contract"],
    "Internal & Client-Specific Docs": ["Client Instructions", "Internal Notes or Checklists", "Email Correspondence", "Service Level Agreements (SLAs)"],
  };

  const SERVICE_CHARGES_MAP = {
    ocean: [
      "Ocean Freight", "ISF (Importer Security Filing)", "THC (Terminal Handling Charges)", "BL Fee (Bill of Lading Fee)",
      "Devanning / Unstuffing", "Customs Inspection Fee", "Port Handling", "Local Transport (Port Drayage)", "Warehouse Storage (Port/CFS)",
      "Detention / Demurrage", "Cargo Insurance", "Customs Brokerage", "Duties & Taxes", "Administrative Fee",
      "Container Cleaning", "Endorsement Fee", "VGM (Verified Gross Mass) Fee", "Documentation Fee", "Port Congestion Surcharge"
    ],
    air: [
      "Air Freight", "AWB Fee (Air Waybill Fee)", "Airport THC (Terminal Handling Charges)", "Airline Handling",
      "Customs Inspection Fee (Airport)", "Airport Handling", "Local Transport (Airport Pickup/Delivery)", "Warehouse Storage (Airport)",
      "Cargo Insurance", "Customs Brokerage", "Duties & Taxes", "Airline Fees (Misc)", "Administrative Fee",
      "Dangerous Goods (DG) Fee", "Fuel Surcharge", "Security Surcharge", "Screening Fee"
    ],
    truck: [
      "Trucking Freight (FTL/LTL)", "Loading / Unloading Fee", "Inland Cargo Insurance", "Escort / Security Fee",
      "Layover / Waiting Time", "Border Crossing Fee", "Special Permits", "Customs Brokerage (Border)",
      "Tolls", "Warehouse Storage (Origin/Destination)", "Forklift Charges", "Palletizing / Shrink-wrapping",
      "Fuel Surcharge (Trucking)", "Lift Gate Service"
    ],
    currencies: ["USD", "MXN"],
    defaultCurrency: "USD"
  };
  ...
  const IN_PROGRESS_STATUSES = ["Pending", "Booked", "In Transit", "At Destination", "On Hold"];
  const SERVICE_TABLES = { ocean: 'ServiceOcean', air: 'ServiceAir', truck: 'ServiceTruck' };
  ...
  const serviceCategoryModalSelect = document.getElementById("serviceCategoryModal");
  ...
  const isfRequiredIndicatorModal = document.getElementById("isfRequiredIndicatorModal");
  const flightNumberGroupModal = document.getElementById("flightNumberGroupModal");
  ...
  const dimensionsCbmGroupModal = document.getElementById("dimensionsCbmGroupModal");
  ...
  const viewTruckSpecificDetails = document.getElementById("viewTruckSpecificDetails");
  ...
  const viewCommodityDescription = document.getElementById("viewCommodityDescription");
  ...
  const editServiceDisplayIdHeader = document.getElementById("editServiceDisplayIdHeader");
  const editServiceCategoryModalSelect = document.getElementById("editServiceCategoryModal");
  ...
  const editOceanServiceTypeModalSelect = document.getElementById("editOceanServiceTypeModal");
  const editOceanServiceModalSelect = document.getElementById("editOceanServiceModal");
  ...
  const editAddIsfLaterModalCheckbox = document.getElementById("editAddIsfLaterModal");
  const editIsfErrorModal = document.getElementById("editIsfErrorModal");
  const editIsfRequiredIndicatorModal = document.getElementById("editIsfRequiredIndicatorModal");
  const editTruckServiceTypeModalSelect = document.getElementById("editTruckServiceTypeModal");
  ...
  const editFlightNumberGroupModal = document.getElementById("editFlightNumberGroupModal");
  const editFlightNumberModalInput = document.getElementById("editFlightNumberModal");
  ...
  const editSealNumberModalInput = document.getElementById("editSealNumberModal");
  const editNumPackagesModalInput = document.getElementById("editNumPackagesModal");
  const editGrossWeightModalInput = document.getElementById("editGrossWeightModal");
  const editDimensionsCbmGroupModal = document.getElementById("editDimensionsCbmGroupModal");
  const editDimensionsModalInput = document.getElementById("editDimensionsModal");
  const editCbmModalInput = document.getElementById("editCbmModal");
  const editCommodityDescriptionModalTextarea = document.getElementById("editCommodityDescriptionModal");
  const editHtsCodeModalInput = document.getElementById("editHtsCodeModal");
  const editServiceStatusModalSelect = document.getElementById("editServiceStatusModal");
  ...
  const dbCompletedServicesEl = document.getElementById("db-completed-services");
  const dbInProgressServicesEl = document.getElementById("db-inprogress-services");
  ...
  const closeArchiveServiceModalBtn = document.getElementById("closeArchiveServiceModalBtn");
  ...
  const archiveServiceTypeSelect = document.getElementById("archiveServiceTypeSelect");
  ...
  const closeProgressNotificationFooterBtn = document.getElementById("closeProgressNotificationFooterBtn");
  const progressNotificationTableBody = document.getElementById("progressNotificationTableBody");
  const noProgressServicesMessage = document.getElementById("noProgressServicesMessage");
  const openProgressNotificationModalBtnManual = document.getElementById("openProgressNotificationModalBtnManual");
  const chargesContainerCreate = document.getElementById("chargesContainerModal_create");
  const addChargeBtnCreate = document.getElementById("addChargeBtn_create");
  const chargesContainerEdit = document.getElementById("chargesContainerModal_edit");
  ...
  function isWithinOperatingHours() {
    ...
    return tijuanaHour >= OPERATING_HOUR_START_TIJUANA && tijuanaHour < OPERATING_HOUR_END_TIJUANA;
  }
  function getCategoryTextAndClass(category) {
    switch (category) {
      case "ocean": return { text: "🌊 Ocean", class: "service-type-ocean", rowClass: "row-type-ocean", prefix: "OCN" };
      case "air": return { text: "✈️ Air", class: "service-type-air", rowClass: "row-type-air", prefix: "AIR" };
      case "truck": return { text: "🚚 Truck", class: "service-type-truck", rowClass: "row-type-truck", prefix: "TRK" };
      default: return { text: "N/A", class: "", rowClass: "", prefix: "SRV" };
    }
  }
  ...
  function getConfirmModalElements() {
    ...
    if (!confirmModalElement || !confirmTitleElement || !confirmMessageElement || !confirmOkBtn || !confirmCancelBtn || !confirmCloseBtn) {
      console.error("ST Module: One or more custom confirm modal elements are missing from the DOM.");
      return false;
    }
  ...
  async function prepareDataForSupabase(formData, category, isNewRecord = false, chargesContainer) {
    if (!currentUserST) {
    ...
    const fieldMapping = {
        ...
        finalDestinationModal: 'final_destination', mblHawbModal: 'mbl_hawb_pro',
  ...
  async function fetchAllServices() {
    ...
    try {
      ...
      const oceanData = oceanRes.data.map(row => transformServiceDataForUI(row, 'ocean'));
      const airData = airRes.data.map(row => transformServiceDataForUI(row, 'air'));
      const truckData = truckRes.data.map(row => transformServiceDataForUI(row, 'truck'));
  ...
  async function uploadServiceDocument(serviceId, serviceCategory, docCategory, docType, file) {
    ...
    if (!currentUserST) {
        showCustomNotification("User not authenticated for this module. Cannot upload file.", "error");
        return null;
    }
  ...
  if (newServiceForm) {
    newServiceForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        Object.keys(serviceDataForDb).forEach(key => { if (serviceDataForDb[key] === undefined) delete serviceDataForDb[key]; });

        if (!serviceDataForDb.documents_metadata) serviceDataForDb.documents_metadata = [];
        ...
        if (serviceCategory === "ocean" && isfFile && isfFile.name && !newService.isf_filed_later) {
            const isfMetadata = await uploadServiceDocument(newService.id, serviceCategory, "Regulatory and Compliance Documents", "ISF Filing Confirmation", isfFile);
            if (isfMetadata) {
                const updatedDocsMetadata = [...(newService.documents_metadata || []), isfMetadata];
                const { error: updateError } = await supabase
                    .from(tableName)
                    .update({ documents_metadata: updatedDocsMetadata, updated_at: new Date().toISOString() })
                    .eq('id', newService.id);
                if (updateError) console.error("Error updating service with ISF metadata:", updateError);
            }
        }
  ...
  if (editServiceForm) {
    editServiceForm.addEventListener("submit", async (event) => {
      event.preventDefault();
      if (!currentUserST) { showCustomNotification("Authentication required to edit service.", "error"); return; }
      ...
      try {
        ...
        const originalService = allServicesData.find(s => s.id === serviceIdToUpdate);
        let finalDocumentsMetadata = originalService ? [...originalService.documents] : [];

        if (serviceCategory === "ocean" && newIsfFile && newIsfFile.name) {
            const isfMetadata = await uploadServiceDocument(serviceIdToUpdate, serviceCategory, "Regulatory and Compliance Documents", "ISF Filing Confirmation", newIsfFile);
            if (isfMetadata) {
        ...
        if (error) {
          console.error(`Error updating ${serviceCategory} service ${serviceIdToUpdate}:`, error);
  ...
  async function deleteService(serviceId, serviceCategoryInternal) {
    if (!currentUserST) { showCustomNotification("Authentication required to delete service.", "error"); return; }
    if (!serviceId || !serviceCategoryInternal) { showCustomNotification("Cannot delete: Service ID or category is missing.", "error"); return; }

    const tableName = SERVICE_TABLES[serviceCategoryInternal];
    if (!tableName) { showCustomNotification(`Cannot delete: Invalid service category "${serviceCategoryInternal}".`, "error"); return; }
    ...
    if (serviceToDelete && serviceToDelete.documents && serviceToDelete.documents.length > 0) {
        ...
        if (filePathsToDelete.length > 0) {
            ...
            if (storageError) {
                console.error("Error deleting documents from storage, but proceeding with DB record deletion:", storageError);
                showCustomNotification(`Warning: Could not delete all associated files: ${storageError.message}`, "warning", 6000);
            }
        }
    }

    const { error } = await supabase.from(tableName).delete().eq('id', serviceId);
  ...
  async function completeService(serviceId, serviceCategoryInternal) {
    ...
    if (!serviceId || !serviceCategoryInternal) { showCustomNotification("Cannot complete: Service ID or category is missing.", "error"); return; }
    const tableName = SERVICE_TABLES[serviceCategoryInternal];
    ...
    if (updateServiceError) {
        console.error(`Error completing service ${serviceId}:`, updateServiceError);
        showCustomNotification(`Error completing service: ${updateServiceError.message}`, "error");
        return;
    }
    ...
    if (insertInvoiceError) {
        console.error(`Error creating invoice for service ${serviceId}:`, insertInvoiceError);
        showCustomNotification(`Service completed, but failed to create invoice: ${insertInvoiceError.message}`, "error", 7000);
    } else {
  ...
  function createChargeRowElement(serviceType, charge = null, isEditable = true) {
    ...
    const availableCurrencies = SERVICE_CHARGES_MAP.currencies || [SERVICE_CHARGES_MAP.defaultCurrency];
    ...
    availableCharges.forEach(chargeName => {
        nameOptionsHtml += `<option value="${chargeName}" ${charge && charge.name === chargeName ? 'selected' : ''}>${chargeName}</option>`;
    });
    ...
    availableCurrencies.forEach(currencyCode => {
        currencyOptionsHtml += `<option value="${currencyCode}" ${charge && charge.currency === currencyCode ? 'selected' : (!charge && currencyCode === SERVICE_CHARGES_MAP.defaultCurrency ? 'selected' : '')}>${currencyCode}</option>`;
    });
  ...
  function resetCreateFormSpecifics() {
    if (oceanFieldsModal) oceanFieldsModal.style.display = "none"; if (airFieldsModal) airFieldsModal.style.display = "none";
  ...
  function handleCreateCategoryChange() {
    ...
    if (category === "ocean") {
      ...
      if (document.getElementById('dimensionsModal')) document.getElementById('dimensionsModal').closest('.st-form-group').style.display = 'block';
      if (document.getElementById('cbmModal')) document.getElementById('cbmModal').closest('.st-form-group').style.display = 'none';
      ...
      if (document.getElementById('dimensionsModal')) document.getElementById('dimensionsModal').closest('.st-form-group').style.display = 'block';
      if (document.getElementById('cbmModal')) document.getElementById('cbmModal').closest('.st-form-group').style.display = 'block';
      if (dimensionsCbmGroupModal) dimensionsCbmGroupModal.style.display = "block";
  ...
  function populateViewModal(serviceData) {
    ...
    viewCommodityDescription.textContent = serviceData.commodityDescription || "N/A";
    ...
    if (creatorInfoEl) creatorInfoEl.textContent = serviceData.user_email || "N/A";
    ...
    if (viewServiceChargesContainer && viewNoChargesMessage) {
        ...
        if (charges.length > 0) {
            ...
            charges.forEach(charge => {
                ...
                const currency = charge.currency || SERVICE_CHARGES_MAP.defaultCurrency;
                totalsByCurrency[currency] = (totalsByCurrency[currency] || 0) + (charge.cost || 0);
            });
  ...
  function handleEditCategoryChange() {
    ...
    const hblLabelEdit = document.querySelector('label[for="editHblModal"]');
     ...
     if (category === "ocean") {
        ...
        if (document.getElementById('editDimensionsModal')) document.getElementById('editDimensionsModal').closest('.st-form-group').style.display = 'block';
        if (document.getElementById('editCbmModal')) document.getElementById('editCbmModal').closest('.st-form-group').style.display = 'none';
        ...
        if (document.getElementById('editDimensionsModal')) document.getElementById('editDimensionsModal').closest('.st-form-group').style.display = 'block';
        if (document.getElementById('editCbmModal')) document.getElementById('editCbmModal').closest('.st-form-group').style.display = 'block';
        if (editDimensionsCbmGroupModal) editDimensionsCbmGroupModal.style.display = "block";
    }
    ...
    if (chargesContainerEdit) {
        ...
        existingChargeRows.forEach(row => {
            ...
            const costInput = row.querySelector('input[name="chargeCost[]"]');
            const currencySelect = row.querySelector('select[name="chargeCurrency[]"]');
            if (nameSelect && costInput && currencySelect && nameSelect.value && !isNaN(parseFloat(costInput.value))) {
                currentCharges.push({ name: nameSelect.value, cost: parseFloat(costInput.value), currency: currencySelect.value });
            }
        });
  ...
  function populateEditModal(serviceData) {
    ...
    editShipperModalInput.value = serviceData.shipper !== "N/A" ? serviceData.shipper : "";
    editConsigneeModalInput.value = serviceData.consignee !== "N/A" ? serviceData.consignee : "";
    editCarrierModalInput.value = serviceData.carrier !== "N/A" ? serviceData.carrier : "";
    editPolModalInput.value = serviceData.pol !== "N/A" ? serviceData.pol : "";
    editPodModalInput.value = serviceData.pod !== "N/A" ? serviceData.pod : "";
    editFinalDestinationModalInput.value = serviceData.finalDestination !== "N/A" ? serviceData.finalDestination : "";
    editMblHawbModalInput.value = serviceData.mblHawbPro !== "N/A" ? serviceData.mblHawbPro : "";
    editNumPackagesModalInput.value = serviceData.numPackages !== "N/A" ? serviceData.numPackages : "";
    editGrossWeightModalInput.value = serviceData.grossWeight && serviceData.grossWeight !== "N/A" ? parseFloat(String(serviceData.grossWeight).replace(' kg','')) : "";
    editCommodityDescriptionModalTextarea.value = serviceData.commodityDescription !== "N/A" ? serviceData.commodityDescription : "";
    editHtsCodeModalInput.value = serviceData.htsCode !== "N/A" ? serviceData.htsCode : "";
    editServiceStatusModalSelect.value = serviceData.status || "Pending";
    editServiceNotesModalTextarea.value = serviceData.notes !== "N/A" ? serviceData.notes : "";
    ...
    if (serviceData.serviceCategoryInternal === "ocean") {
        ...
        editContainerTypeModalInput.value = serviceData.containerType !== "N/A" ? serviceData.containerType : "";
        ...
        editCbmModalInput.value = serviceData.cbm !== "N/A" ? serviceData.cbm : "";
        editDimensionsModalInput.value = serviceData.dimensions !== "N/A" ? serviceData.dimensions : "";
    }
    populateChargesUI(chargesContainerEdit, serviceData.serviceCategoryInternal, serviceData.service_charges || [], true);
    validateIsfEditModal();
  }
  ...
  async function handleDocumentUpload() {
    if (!currentServiceIdForDocs || !docCategorySelect || !docTypeSelect || !docFileInput || !docFileInput.files[0]) {
      showCustomNotification("Please select service, category, type, and file.", "warning"); return;
    }
    ...
    if (!service) {
      ...
      if(uploadButton) uploadButton.disabled = false; return;
    }
    ...
    try {
        ...
        if (newDocumentMetadata) {
            ...
            const updatedDocumentsMetadata = [...currentDocuments, newDocumentMetadata];

            const tableName = SERVICE_TABLES[service.serviceCategoryInternal];
            const { error: updateError } = await supabase
                .from(tableName)
                .update({ documents_metadata: updatedDocumentsMetadata, updated_at: new Date().toISOString() })
  ...
  async function handleDocumentAction(event) {
    ...
    } else if (targetButton.classList.contains("st-doc-card-delete") && docPath && docIdInMetadata) {
        showCustomConfirm("Confirm Document Deletion", `Are you sure you want to delete the document "${docFileName}"? This action cannot be undone.`,
            async () => {
                try {
                    ...
                    const currentDocuments = service.documents || [];
                    const updatedDocumentsMetadata = currentDocuments.filter(doc => doc.id !== docIdInMetadata);
                    const tableName = SERVICE_TABLES[service.serviceCategoryInternal];
  ...
  const commonColumnConfig = {
    ...
    service_display_id: { title: "Service ID", data: "service_display_id", className: "dt-left" },
    user_email: { title: "Created By", data: "user_email", className: "dt-left" },
    ...
    finalDestination: { title: "Final Dest.", data: "finalDestination", className: "dt-left" },
    numPackages: { title: "# Packages", data: "numPackages", type: "num", className: "dt-right" },
    grossWeight: { title: "Gross Wt.", data: "grossWeight", type: "num", className: "dt-right" },
    commodityDescription: { title: "Commodity", data: "commodityDescription", className: "dt-left" },
  ...
  const columnDefinitions = {
    all: [
      ...
      commonColumnConfig.mblHawbPro, commonColumnConfig.status,
    ...
    ocean: [
      ...
      commonColumnConfig.grossWeight, commonColumnConfig.commodityDescription, commonColumnConfig.htsCode,
    ...
    air: [
        ...
        {...commonColumnConfig.pod, title: "POD (Air)"}, commonColumnConfig.finalDestination,
        {...commonColumnConfig.mblHawbPro, title: "MAWB"}, commonColumnConfig.hawb, commonColumnConfig.numPackages,
        commonColumnConfig.grossWeight, commonColumnConfig.dimensions, commonColumnConfig.commodityDescription,
        commonColumnConfig.htsCode, commonColumnConfig.status,
        commonColumnConfig.manageStatusActions, commonColumnConfig.viewEditActions, commonColumnConfig.docsAction,
        commonColumnConfig.internal_id,
    ],
    truck: [
  ...
  function getCurrentMonthRange() {
    ...
    const nextMonthFirstDay = new Date(Date.UTC(year, month + 1, 1, 0, 0, 0, 0));
    const lastDayCurrentMonth = new Date(nextMonthFirstDay.getTime() - 1);
    const displayEndDate = new Date(Date.UTC(lastDayCurrentMonth.getUTCFullYear(), lastDayCurrentMonth.getUTCMonth(), lastDayCurrentMonth.getUTCDate(), 23, 59, 59, 999));

    const todayUTC = new Date(Date.UTC(year, month, now.getUTCDate(), 0, 0, 0, 0));
  ...
  function filterServicesForCurrentMonthDisplay(services) {
    ...
    return services.filter((service) => {
      if (COMPLETED_STATUSES.includes(service.status) || service.status === CANCELLED_STATUS) {
          return false;
      }

      const etdString = service.etd && service.etd !== "N/A" ? service.etd : null;
      const etaString = service.eta && service.eta !== "N/A" ? service.eta : null;

      const etdDate = etdString ? new Date(etdString.includes('T') || etdString.includes('Z') ? etdString : etdString + 'T00:00:00Z') : null;
      const etaDate = etaString ? new Date(etaString.includes('T') || etaString.includes('Z') ? etaString : etaString + 'T00:00:00Z') : null;

      const etdInMonth = etdDate && etdDate >= startDate && etdDate <= displayEndDate;
      const etaInMonth = etaDate && etaDate >= startDate && etaDate <= displayEndDate;
  ...
  function updateDashboard(servicesForDashboard) {
    ...
    dbOceanServicesEl.textContent = currentMonthServicesForDashboard.filter(s => s.serviceCategoryInternal === "ocean").length;
    dbAirServicesEl.textContent = currentMonthServicesForDashboard.filter(s => s.serviceCategoryInternal === "air").length;
    dbTruckServicesEl.textContent = currentMonthServicesForDashboard.filter(s => s.serviceCategoryInternal === "truck").length;

    const { startDate: monthStart, displayEndDate: monthEnd } = getCurrentMonthRange();
    dbCompletedServicesEl.textContent = servicesForDashboard.filter(s => {
        if (!COMPLETED_STATUSES.includes(s.status)) return false;
        const etdDate = s.etd && s.etd !== "N/A" ? new Date(s.etd + 'T00:00:00Z') : null;
        const etaDate = s.eta && s.eta !== "N/A" ? new Date(s.eta + 'T00:00:00Z') : null;
        return (etdDate && etdDate >= monthStart && etdDate <= monthEnd) ||
  ...
  function handleFilterArchive() {
    ...
    const dataForArchive = allServicesData.filter(service => {
        ...
        const etdDate = service.etd && service.etd !== "N/A" ? new Date(service.etd.includes('T') || service.etd.includes('Z') ? service.etd : service.etd + 'T00:00:00Z') : null;
        const etaDate = service.eta && service.eta !== "N/A" ? new Date(service.eta.includes('T') || service.eta.includes('Z') ? service.eta : service.eta + 'T00:00:00Z') : null;

        const dateMatchesSelection = (date) => date && date.getUTCFullYear() === selectedYear && date.getUTCMonth() === selectedMonth;

        return (etdDate && dateMatchesSelection(etdDate)) || (etaDate && dateMatchesSelection(etaDate));
    });
  ...
  function showProgressNotificationModal(isManualCall = false) {
    ...
    if (!isManualCall) {
        const criticalModals = [createServiceModal, viewServiceModal, editServiceModal, docManagementModal, archiveServiceModal, confirmModalElement];
        if (criticalModals.some(modal => modal && modal.classList.contains('st-modal-open'))) {
          console.log("ST Module: Progress notification skipped, a critical modal is open.");
          return;
        }
    }
  ...
  function initCreateModalListeners() {
    ...
    if (addChargeBtnCreate && chargesContainerCreate && serviceCategoryModalSelect) {
        addChargeBtnCreate.addEventListener('click', () => {
            const currentServiceType = serviceCategoryModalSelect.value;
            if (!currentServiceType) {
                showCustomNotification("Please select a service type first to add charges.", "warning");
                return;
            }
            chargesContainerCreate.appendChild(createChargeRowElement(currentServiceType, null, true));
        });
    }
  }
  function initViewModalListeners() {
    if (closeViewServiceModalBtn) closeViewServiceModalBtn.addEventListener("click", () => closeModal(viewServiceModal));
    if (closeViewModalFooterBtn) closeViewModalFooterBtn.addEventListener("click", () => closeModal(viewServiceModal));
    if (viewServiceModal) viewServiceModal.addEventListener("click", (event) => { if (event.target === viewServiceModal) closeModal(viewServiceModal); });
  }
  function initEditModalListeners() {
    if (closeEditServiceModalBtn) closeEditServiceModalBtn.addEventListener("click", () => closeModal(editServiceModal, editServiceForm));
    if (cancelEditServiceBtn) cancelEditServiceBtn.addEventListener("click", () => closeModal(editServiceModal, editServiceForm));
    if (editServiceModal) editServiceModal.addEventListener("click", (event) => { if (event.target === editServiceModal) closeModal(editServiceModal, editServiceForm); });
    if (editAddIsfLaterModalCheckbox) editAddIsfLaterModalCheckbox.addEventListener("change", validateIsfEditModal);
    ...
    if (addChargeBtnEdit && chargesContainerEdit && editServiceCategoryModalSelect) {
        addChargeBtnEdit.addEventListener('click', () => {
            ...
            chargesContainerEdit.appendChild(createChargeRowElement(currentServiceType, null, true));
        });
    }
  }
  ...
  function initArchiveModalListeners() {
    ...
    if (closeArchiveServiceModalBtn) closeArchiveServiceModalBtn.addEventListener("click", () => closeModal(archiveServiceModal));
    if (closeArchiveModalFooterBtn) closeArchiveModalFooterBtn.addEventListener("click", () => closeModal(archiveServiceModal));
    if (archiveServiceModal) archiveServiceModal.addEventListener("click", (event) => { if (event.target === archiveServiceModal) closeModal(archiveServiceModal); });
    if (filterArchiveBtn) filterArchiveBtn.addEventListener("click", handleFilterArchive);
  }
  function initProgressNotificationModalListeners() {
    if (closeProgressNotificationModalBtn) closeProgressNotificationModalBtn.addEventListener("click", () => closeModal(progressNotificationModal));
    if (closeProgressNotificationFooterBtn) closeProgressNotificationFooterBtn.addEventListener("click", () => closeModal(progressNotificationModal));
  ...
  function initModalListeners() {
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape") {
        const openModals = Array.from(document.querySelectorAll('.st-modal.st-modal-open'))
                                .sort((a,b) => parseInt(b.style.zIndex || 0) - parseInt(a.style.zIndex || 0));
        if (openModals.length > 0) {
            const topModal = openModals[0];
            if (topModal === createServiceModal) closeModal(createServiceModal, newServiceForm);
            else if (topModal === viewServiceModal) closeModal(viewServiceModal);
            else if (topModal === editServiceModal) closeModal(editServiceModal, editServiceForm);
            else if (topModal === docManagementModal) closeModal(docManagementModal);
            else if (topModal === archiveServiceModal) closeModal(archiveServiceModal);
            else if (topModal === progressNotificationModal) closeModal(progressNotificationModal);
  ...
  function handleTableActions(event, tableInstance, dataArray) {
      ...
      if (!rowElement) { console.warn("handleTableActions: Could not find TR element for action button"); return; }
      ...
      switch (action) {
        ...
        case "edit":
            ...
            break;
        case "complete":
          if (COMPLETED_STATUSES.includes(serviceToProcess.status) || serviceToProcess.status === CANCELLED_STATUS) {
            showCustomNotification("This service is already completed or cancelled.", "info"); return;
          }
          showCustomConfirm("Confirm Completion", `Are you sure you want to mark service <strong>${displayIdForModalsAndConfirm}</strong> as "Completed"? This will prepare its invoice data.`,
            () => completeService(serviceToProcess.id, serviceToProcess.serviceCategoryInternal)
          );
          break;
        case "delete":
          const categoryInfoDel = getCategoryTextAndClass(serviceToProcess.serviceCategoryInternal);
          showCustomConfirm("Confirm Deletion", `Are you sure you want to delete service <strong>${displayIdForModalsAndConfirm} (${categoryInfoDel.text})</strong>? Associated documents will also be deleted. This action cannot be undone.`,
            () => deleteService(serviceToProcess.id, serviceToProcess.serviceCategoryInternal)
          );
          break;
